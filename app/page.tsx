...
const Divider = () => (
  <div className="w-full flex justify-center py-4 bg-warm-bg">
    <div className="w-24 h-[1px] bg-warm-beige/50" />
  </div>
);
...
export default function Home() {
  ...
  // This function is triggered by the Curtain when the logo is clicked
  const startExperience = () => {
    // The !hasStarted check ensures the music ONLY plays once and never restarts
    if (!hasStarted && audioRef.current) {
      setHasStarted(true);
      setIsPlaying(true);
      
      audioRef.current.play().catch((e) => {
        console.log("Audio play blocked by browser, user must interact:", e);
      });
    }
  };
      ...
      <main className="w-full overflow-x-hidden bg-warm-bg relative font-sans">
        ...
        <section className="min-h-screen w-full flex flex-col items-center justify-center px-4 py-20">
          <ScrollReveal variant="fadeUp">
            <p className="text-center uppercase tracking-[0.3em] text-[10px] text-warm-accent mb-12 font-sans">The Celebration Begins In</p>
            <Countdown />
          </ScrollReveal>
        </section>
